...
/** TCPROS implements the node-to-node communication once the ROC
    Master has connected us with another node (a subscriber to one of
    our topics or a publisher to a topic we are subscribed to) */
TCPROS = function(options) {
  options = options || {};
  this.node        = options.node;
  this.topic       = options.topic; // or service
  ...
  portscanner.findAPortNotInUse(9000, null, hostname, function(error, port) {
    ...
    var server = net.createServer(function(socket) {
      ...
      socket.on('data', function(data) {
        // console.log("createPublisher data", data.toString('hex'));
...
/** This is the socket used for subscriptions, i.e., the data it
    receives are messages */
TCPROS.prototype.createSubscriber = function(port, host) {
  ...
  // console.log("createSubscriber", connectionHeader);
  var buffer = serializeConnectionHeader(connectionHeader);
  this.subscriberSocket.write(buffer)
};

/** Make a service call */
TCPROS.prototype.callService = function(
  port, host, service, message, callback) {

  var socket = net.createConnection(port, host)
  socket.on('data', function(data) {
    var connectionHeader = deserializeConnectionHeader(data);
    // console.log("service response:", data.toString('ascii'), connectionHeader);
    socket.end();
    callback(null, connectionHeader);
  });
  ...
  socket.on('end', function(e, r) {console.log("end", e, r);});
  socket.on('close', function(e, r) {console.log("close", e, r);});
  ...
  socket.on('connect', function() {
    ...
    var buffer = serializeConnectionHeader(connectionHeader, true);
    socket.write(buffer)
    // console.log("header", buffer.toString('ascii'));
...
function serializeConnectionHeader(connectionHeader, service) {
  var headers = [];
  ...
  if (service) {
    var service = connectionHeader.service;
    if (service !== undefined) {
      headers.push({ key: 'service', value: service });
    }
  } else {
...
function deserializeInnerMessage(message, buffer, bufferOffset) {
  message.fields.forEach(function(field) {
    // console.log("deserializeInnerMessage", field.name);
    var fieldValue = message[field.name];
